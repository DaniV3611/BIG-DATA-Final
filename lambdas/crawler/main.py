...
def get_crawler_status():
    ...
    try:
        ...
        return {
            ...
            'creation_time': (
                response['Crawler']['CreationTime'].isoformat()
                if 'CreationTime' in response['Crawler'] else None
            )
        }
    except ClientError:
    ...
    try:
        ...
        try:
            ...
            for i, partition in enumerate(partitions[:5]):  # Show first 5 partitions
                ...
                logger.info(
                    f"  Partition {i+1}: periodico={values[0]}, year={values[1]}, "
                    f"month={values[2]}, day={values[3]} -> {location}"
                )
